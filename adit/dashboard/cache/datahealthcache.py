...
import logging
import numpy as np
from adit.controllers import TileDBController, EventLoopController, PPOOL
...
class DataMonitorCache:
    ...
    async def _update_pair(self, pair):
        ...
        latest_timestamp = await self.evl_loop.run_in_executor(None, self.get_checkpoint, pair)
        if latest_timestamp != self.metadata['latest_timestamp'][pair]:
            ...
            data = await self.evl_loop.run_in_executor(None, self.get_ts_data, pair, from_ts, to_ts)
