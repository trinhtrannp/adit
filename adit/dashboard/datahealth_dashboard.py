...
class DataHealthDashboard(DashboardComponent):
    ...
    def get_data(self, pair):
        ...
        self.logger.debug(f"Got new data from cache pair={pair} d={d}")
        return d
...
def datahealth_doc(worker, extra, doc):
    with log_errors():
        datamonitor = DataHealthDashboard(worker, sizing_mode="stretch_both")
...
def init(mode):
    ...
    if target is not None:
        target.template_variables["pages"].append("adit-datahealth")
        target.applications["/adit-datahealth"] = datahealth_doc
